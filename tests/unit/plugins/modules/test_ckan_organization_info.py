...
from ansible_collections.msradam.ckan.plugins.modules import ckan_organization_info as mod

MOD_PATH = 'ansible_collections.msradam.ckan.plugins.modules.ckan_organization_info'
...
_ORG = {'id': 'org-uuid', 'name': 'env-agency', 'title': 'Environment Agency', 'state': 'active'}
...
def test_run_returns_org_when_found():
    with patch(MOD_PATH + '.AnsibleModule') as mock_cls, \
        ...
        c.show.return_value = dict(_ORG)
        with pytest.raises(SystemExit) as exc:
            mod.run_module()
    assert exc.value.code == 0
    assert m.exit_json.call_args[1]['changed'] is False
    assert m.exit_json.call_args[1]['organization']['name'] == 'env-agency'
...
def test_run_uses_organization_show_action():
    with patch(MOD_PATH + '.AnsibleModule') as mock_cls, \
         patch(MOD_PATH + '.CKANClient') as mock_client_cls:
        m = _mock(dict(_BASE_PARAMS))
        mock_cls.return_value = m
        ...
        with pytest.raises(SystemExit):
            mod.run_module()
    assert c.show.call_args[0][0] == 'organization_show'
